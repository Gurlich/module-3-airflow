...
for rocket_type in rockets:
    get_rocket = "python3 /root/airflow/dags/spacex/load_launches.py -y {{ execution_date.year }} -o /var/data -r " + rocket_type,
    if rocket_type == "all":
        get_rocket = "python3 /root/airflow/dags/spacex/load_launches.py -y {{ execution_date.year }} -o /var/data"
        
    t1 = BashOperator(
        task_id="get_data_" + rocket_type, 
        bash_command=get_rocket 
        dag=dag
    )
